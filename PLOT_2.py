...
def generate_physical_solution(alpha, kappa):
    num_steps = 5000
    L = 6.0  # from -3 to 3
    N = 100
    T = 1
...
if __name__ == '__main__':
    ...
    alpha_true = torch.tensor([3, 1, 1, 1.0]).float()
    kappa_true = torch.tensor([2.5, -2, -2, 1.0]).float()
    real_solution = generate_physical_solution(alpha_true, kappa_true)

    model_state_dict = torch.load('models/adv_diff/phys.pt')
